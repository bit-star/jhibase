...
export class UucDepartmentTreeMpService {
  ...
  create(uucDepartmentTree: IUucDepartmentTreeMp): Observable<EntityResponseType> {
    return this.http.post<IUucDepartmentTreeMp>(this.resourceUrl, uucDepartmentTree, { observe: 'response' });
  }

  update(uucDepartmentTree: IUucDepartmentTreeMp): Observable<EntityResponseType> {
    return this.http.put<IUucDepartmentTreeMp>(this.resourceUrl, uucDepartmentTree, { observe: 'response' });
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http.get<IUucDepartmentTreeMp>(`${this.resourceUrl}/${id}`, { observe: 'response' });
  }

  query(req?: any): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http.get<IUucDepartmentTreeMp[]>(this.resourceUrl, { params: options, observe: 'response' });
  }
